"""User-facing wrapper: fit an American free-boundary PINN, then price and
inspect its PDE residual in real (S, K, tau) units instead of normalized
(m, tau). Mirrors `EuropeanPINN`'s interface exactly."""
...
from .train_american import train_american_pinn

__all__ = ["AmericanPINN"]
...
class AmericanPINN:
    ...
    def __init__(self, config: AmericanPINNConfig) -> None:
        self.config = config
        self.model: PINN | None = None
        self.history: dict[str, list[float]] | None = None

    def fit(self) -> "AmericanPINN":
        self.model, self.history = train_american_pinn(self.config)
        self.model.eval()
        return self
    ...
    def pde_residual(self, S: np.ndarray | float, K: np.ndarray | float, tau: np.ndarray | float) -> np.ndarray:
        """L[u] at (S, tau) -- the same Black-Scholes operator as the European
        case. Near zero in the continuation region where the model has
        learned the dynamics well; in the exercise region it's allowed (and
        expected) to be positive -- see `free_boundary.py` equation (3)."""
        model = self._check_fitted()
        S_arr = np.atleast_1d(np.asarray(S, dtype=np.float64))
        K_arr = np.atleast_1d(np.asarray(K, dtype=np.float64))
        tau_arr = np.atleast_1d(np.asarray(tau, dtype=np.float64))
